...
<%@ taglib prefix="estc" uri="http://icts.uiowa.edu/ESTCTagLib"%>
	...
	<sql:query var="locations" dataSource="jdbc/ESTCTagLib">
		select locational,location_id,location.location,count
		from navigation.location_in_by_year,navigation.location
		where person_id = ?::int and pubyear = ?::int and sublocation_id = ?::int  and location_in_by_year.location_id=location.lid;
	...
	<c:forEach items="${locations.rows}" var="row" varStatus="rowCounter">
		<li>${row.locational}: <a href="../locations/location.jsp?lid=${row.location_id}">${row.location}</a> (${row.count})
				<jsp:include page="sublocations.jsp">
					...
					<jsp:param name="child" value="${row.location_id}"/>
